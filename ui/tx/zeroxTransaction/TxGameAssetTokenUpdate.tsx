...
import GameAssetTokenUpdateTable from 'ui/shared/GameAssetTokenUpdate/GameAssetTokenUpdateTable';
...
interface Props {
  gameAssetTokenUpdateQuery: GameAssetTokenUpdateQuery;
  tokenTransferFilter?: (data: TokenTransfer) => boolean;
}

const TxGameAssetTokenUpdate = ({ gameAssetTokenUpdateQuery }: Props) => {
  ...
  const tokenUpdates = React.useMemo(() => {
    return gameAssetTokenUpdateQuery.data ? gameAssetTokenUpdateQuery.data.updatedTokens.slice(0, 8) : [];
  }, [ gameAssetTokenUpdateQuery.data ]);
  // useEffect(() => {
  ...
  if (gameAssetTokenUpdateQuery.isError) {
    // setTypeFilter('');
  ...
  const content = tokenUpdates ? (
    <>
      <Hide below="lg" ssr={ false }>
        <GameAssetTokenUpdateTable
          query={ gameAssetTokenUpdateQuery }
          // top={ isActionBarHidden ? 0 : 80 }
        />
      </Hide>
      <Show below="lg" ssr={ false }>
        <GameAssetTokenUpdateList data={ tokenUpdates } isLoading={ gameAssetTokenUpdateQuery.isPlaceholderData }/>
      </Show>
    </>
  ) : null;
  ...
  return (
    <DataListDisplay
      isError={ gameAssetTokenUpdateQuery.isError }
      items={ tokenUpdates }
...
export default TxGameAssetTokenUpdate;
